...
void CamThread::run()
{
    while(1)
    {
        ...
        if(_isCalibrating)
        {
            cv::Mat imageCopy;
            // detect markers
            std::vector<aruco::Marker> detected_markers = TheMarkerDetector.detect(_img);
...
void CamThread::CalAruco(int method, int fast_mode)
{
    ...
    if(_tracker.size() == 1)
    {
        ...
        if(tvec.size().width == 3 || tvec.size().height == 3)
        {
            _camMsg.label_id[0] = _tracker.begin()->first;
            _camMsg.x[0] = tvec.at<double>(0,0);
            _camMsg.y[0] = tvec.at<double>(1,0);
            _camMsg.z[0] = tvec.at<double>(2,0);
            _camMsg.q1[0] = revc.at<double>(0,0);
            _camMsg.q2[0] = revc.at<double>(1,0);
            _camMsg.q3[0] = revc.at<double>(2,0);
        }
    }
    else if(_tracker.size() >= 2)
    {
        ...
        auto tvec = _tracker[id_max_first].getTvec();
        auto revc = _tracker[id_max_first].getRvec();
        if(tvec.size().width == 3 || tvec.size().height == 3)
        {
            ...
            _camMsg.q2[0] = revc.at<double>(1,0);
            _camMsg.q3[0] = revc.at<double>(2,0);
            tvec = _tracker[id_max_second].getTvec();
            revc = _tracker[id_max_second].getRvec();
            _camMsg.label_id[1] = id_max_second;
            _camMsg.x[1] = tvec.at<double>(0,0);
            _camMsg.y[1] = tvec.at<double>(1,0);
            _camMsg.z[1] = tvec.at<double>(2,0);
            _camMsg.q1[1] = revc.at<double>(0,0);
            _camMsg.q2[1] = revc.at<double>(1,0);
            _camMsg.q3[1] = revc.at<double>(2,0);
        }
    }
}
...
bool CamThread::PostCalibration()
{
    ...
    _calibrator.getCalibrationResults(_camParam);
